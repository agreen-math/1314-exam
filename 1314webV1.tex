\documentclass[addpoints]{exam}

\usepackage[utf8]{inputenc}
\usepackage{array}
\usepackage{graphicx}
\usepackage{multicol}
\usepackage{amsmath}
\usepackage{tikz}
\usetikzlibrary{arrows}

\renewcommand*\half{.5}

\setlength\answerlinelength{3in}

%%%%% Question Info %%%%% 

%\printanswers

%%%%% Header and Footer %%%%% 

\pagestyle{headandfoot}
\runningheadrule
\firstpageheader{Math 1314}{Non-Comprehensive Final Exam}{Fall 2025}
\runningheader{Math 1314}
{Non-Comprehensive Final Exam}
{Fall 2025}
\runningheadrule
\firstpagefooter{}{}{}
\runningfooter{}{Page \thepage\ of \numpages}{}

%%%%% Questions %%%%% 

\begin{document}

\uplevel{Number of Questions: \numquestions\hspace{\stretch{1}} Point Total: \numpoints}
\begin{questions}
\question[5] Fill in the table to give an equivalent equation in the specified form.\\
{\centering {\renewcommand{\arraystretch}{3}
\[
\begin{array}{|c|c|} \hline  \textbf{Exponential Form} & \textbf{Logarithmic Form} \\ \hline  k = 3^{4} & \hspace{75mm} \\ \hline  3^{2} = w & \hspace{75mm} \\ \hline  \hspace{75mm} & \ln(7) = t \\ \hline  \hspace{75mm} & \log_{6}(216) = k \\ \hline \end{array}
\]
}}
\begin{solution}
\[\begin{array}{|c|c|} \hline \rule[-0.8em]{0pt}{2.2em} \textbf{Exponential Form} & \textbf{Logarithmic Form} \\ \hline \rule[-1.2em]{0pt}{3em} k = 3^{4} & \log_{3}(k) = 4 \\ \hline \rule[-1.2em]{0pt}{3em} 3^{2} = w & \log_{3}(w) = 2 \\ \hline \rule[-1.2em]{0pt}{3em} e^{t} = 7 & \ln(7) = t \\ \hline \rule[-1.2em]{0pt}{3em} 6^{k} = 216 & \log_{6}(216) = k \\ \hline \end{array}\]
\end{solution}
\vspace{\stretch{1}}

\question Use properties of logarithms to expand or condense the logarithmic expression as specified.
\begin{parts}
    \part[5] Rewrite as the sum or difference of logarithms. Express all powers as factors.\\

    \(\displaystyle{ \displaystyle{\log_{5}\left( \frac{625y^{2}}{25z} \right)} }\)
    \begin{solution}
    \[\log_{5}(625) + 2\log_{5}(y) - \log_{5}(25) - \log_{5}(z)\]

\[4 + 2\log_{5}(y) - 2 - \log_{5}(z)\]

\[\boxed{ 2 + 2\log_{5}(y) - \log_{5}(z) }\]
    \end{solution}
    \vspace{\stretch{1}}\answerline

    \part[5] Rewrite as a single logarithm. Express all factors as powers.\\
    
    \(\displaystyle{ 3\log z + 4\log y - 2\log z - \displaystyle{\frac{1}{3}}\log b }\)
    \begin{solution}
    \[\log(z^{3}) + \log(y^{4}) - \log(z^{2}) - \log(b^{1/3})\]

\[\log\left(\displaystyle{\frac{y^{4} z^{3}}{\sqrt[3]{b} z^{2}}}\right)\]

\[\boxed{ \log\left(\displaystyle{\frac{y^{4} z}{\sqrt[3]{b}}}\right) }\]
    \end{solution}
    \vspace{\stretch{1}}\answerline
\end{parts}
\newpage

\uplevel{For questions \ref{exact-start} through~\ref{exact-end}, solve for \textbf{all} solutions. Identify any extraneous solutions.}
\question[6] \label{exact-start}
\(\displaystyle{ e^{-4 \, x} \cdot e^{x} = e }\)
\begin{solution}
\[\begin{aligned} e^{-4 \, x+x} &= e \\ -4 \, x+x &= 1 \\ -3 \, x &= 1 \\ x &= -\frac{1}{3} \end{aligned}\]

solutions: \(\boxed{ -\frac{1}{3} }\)

extraneous solutions: \(\boxed{ \text{none} }\)
\end{solution}
\vspace{\stretch{1}}
\par solutions: \fillin\fillin \hspace{\stretch{1}} extraneous solutions: \fillin\fillin

\question[6] 
\(\displaystyle{ \log_{2}(x - 2) + \log_{2}(x - 5) = 2 }\)
\begin{solution}
\[\begin{aligned} \log_{2}((x - 2)(x - 5)) &= 2 \\ 4 &= (x - 2)(x - 5) \\ 4 &= x^{2} - 7 \, x + 10 \\ 0 &= x^{2} - 7 \, x + 6 \\ 0 &= (x - 6)(x - 1) \end{aligned}\]

 \(x = 6\) is a valid solution, while \(x = 1\) causes a non-positive argument in the original logarithms. 

solutions: \(\boxed{ x = 6 }\)

extraneous solutions: \(\boxed{ x = 1 }\)
\end{solution}
\vspace{\stretch{1}}
\par solutions: \fillin\fillin \hspace{\stretch{1}} extraneous solutions: \fillin\fillin\newpage

\question[6] \label{exact-end}
\(\displaystyle{ \log_{3}(2) + \log_{3}(5x^2) = \log_{3}(360) }\)
\begin{solution}
\[\begin{aligned} \log_{3}(2 \cdot 5x^2) &= \log_{3}(360) \\ 10x^2 &= 360 \\ x^2 &= 36 \\ x &= \pm 6 \end{aligned}\]

 Both solutions are valid since substituting either into the original equation results in strictly positive arguments for the logarithms. 

solutions: \(\boxed{ x = \pm 6 }\)

extraneous solutions: \(\boxed{ \text{none} }\)
\end{solution}
\vspace{\stretch{1}}
\par solutions: \fillin\fillin \hspace{\stretch{1}} extraneous solutions: \fillin\fillin

\question[6] A savings account earns 4\% annual interest compounded quarterly. An initial deposit of \$4,700 is made, where $t$ represents time in years and $A(t)$ represents the account balance in dollars. Determine how long it will take for the account balance to reach \$9,400,, given:

\(\displaystyle{ \displaystyle{A(t) = 4,700 \left( 1 + \frac{0.04}{4} \right)^{4t}} }\)
\begin{solution}
\[\begin{aligned} 9,400 &= 4,700 \left( 1 + \frac{0.04}{4} \right)^{4t} \\ 2 &= (1.01)^{4t} \\ \log_{1.01}(2) &= 4t \\ \frac{\log_{1.01}(2)}{4} &= t \\ 17.415179... &\approx t \end{aligned}\]

\(\boxed{\text{about } 17.4 \text{ years}}\)
\end{solution}
\vspace{\stretch{1}}\answerline
\newpage

\question[5] Use the table to identify the transformations described by \(\displaystyle{ f(x) = \log_{3}(x + 8) - 4 }\). Circle the option that applies and fill in the blanks as appropriate to describe the transformations on the given function. If one does not apply, you may leave it blank. Then sketch the graph of the transformed function on the grid below. Indicate any asymptotes with dashed lines.\\
\begin{table}[h]
    \renewcommand{\arraystretch}{3} 
    \centering
    \begin{tabular}{|l|l|}
        \hline
        \textbf{Horizontal Transformations} & \textbf{Vertical Transformations} \\ \hline
        Reflection: YES or NO & Reflection: YES or NO \\ \hline
        Dilation: \underline{\hspace{2cm}} times as wide & Dilation: \underline{\hspace{2cm}} times as tall \\ \hline
        Translation: \underline{\hspace{2cm}} units LEFT or RIGHT & Translation: \underline{\hspace{2cm}} units UP or DOWN \\ \hline
    \end{tabular}
\end{table}
\begin{solution}
\begin{itemize}
\item
 \(\textbf{Horizontal: } \text{Reflection: NO, Dilation: 1, Shift: 8 units LEFT.}\) 

\item
 \(\textbf{Vertical: } \text{Reflection: NO, Dilation: 1, Shift: 4 units DOWN.}\) 

\end{itemize}


 \[\begin{tikzpicture}[scale=0.39] \draw[step=1cm, gray!40, very thin] (-10,-10) grid (10,10); \draw[thick, <->] (-10.5,0) -- (10.5,0); \draw[thick, <->] (0,-10.5) -- (0,10.5); \clip (-10.5,-10.5) rectangle (10.5,10.5);\draw[dashed, red, thick] (-8, -10.5) -- (-8, 10.5);\draw[blue, very thick, samples=100, domain=-7.99000000000000:10.5000000000000, smooth] plot (\x, {(ln(\x - (-8))/ln(3)) + (-4)});\end{tikzpicture}\]
\end{solution}

\begin{center}
\scalebox{0.75}{
    \begin{tikzpicture}[>=triangle 45]
      \draw [step=.5cm, style={black!60}] (-5.4,-5.4) grid (5.4,5.4);
      \draw [->, line width=2] (-5.5,0) -- (5.5,0); 
      \draw [->, line width=2] (0,-5.5) -- (0,5.5);
    \end{tikzpicture}
}
\end{center}
\newpage

\question[5] Use the table to identify the transformations described by \(\displaystyle{ f(x) = 2^{x - 6} - 3 }\). Circle the option that applies and fill in the blanks as appropriate to describe the transformations on the given function. If one does not apply, you may leave it blank. Then sketch the graph of the transformed function on the grid below. Indicate any asymptotes with dashed lines.\\
\begin{table}[h]
    \renewcommand{\arraystretch}{3} 
    \centering
    \begin{tabular}{|l|l|}
        \hline
        \textbf{Horizontal Transformations} & \textbf{Vertical Transformations} \\ \hline
        Reflection: YES or NO & Reflection: YES or NO \\ \hline
        Dilation: \underline{\hspace{2cm}} times as wide & Dilation: \underline{\hspace{2cm}} times as tall \\ \hline
        Translation: \underline{\hspace{2cm}} units LEFT or RIGHT & Translation: \underline{\hspace{2cm}} units UP or DOWN \\ \hline
    \end{tabular}
\end{table}
\begin{solution}
\begin{itemize}
\item
 \(\textbf{Horizontal: } \text{Reflection: NO, Dilation: 1, Shift: 6 units RIGHT.}\) 

\item
 \(\textbf{Vertical: } \text{Reflection: NO, Dilation: 1, Shift: 3 units DOWN.}\) 

\end{itemize}


 \[\begin{tikzpicture}[scale=0.39] \draw[step=1cm, gray!40, very thin] (-10,-10) grid (10,10); \draw[thick, <->] (-10.5,0) -- (10.5,0); \draw[thick, <->] (0,-10.5) -- (0,10.5); \clip (-10.5,-10.5) rectangle (10.5,10.5);\draw[dashed, red, thick] (-10.5, -3) -- (10.5, -3);\draw[blue, very thick, samples=100, domain=-10.5000000000000:10.5000000000000, smooth] plot (\x, {pow(2, \x - (6)) + (-3)});\end{tikzpicture}\]
\end{solution}

\begin{center}
\scalebox{0.75}{
    \begin{tikzpicture}[>=triangle 45]
      \draw [step=.5cm, style={black!60}] (-5.4,-5.4) grid (5.4,5.4);
      \draw [->, line width=2] (-5.5,0) -- (5.5,0); 
      \draw [->, line width=2] (0,-5.5) -- (0,5.5);
    \end{tikzpicture}
}
\end{center}
\newpage

\question[6] A bakery is selling loaves of bread and pies. On the first day of sales, they sold 40 loaves of bread and 10 pies for a total of \$320. They took in \$640 for 40 loaves of bread and 30 pies on the second day. Find the price of each type of item.
\vspace{\stretch{1}}\\
loaf of bread:\fillin \hspace{\stretch{1}} pie:\fillin
\begin{solution}
\[\begin{aligned} \text{Let } b &\text{ = price of loaf of bread} \\ \text{Let } p &\text{ = price of pie} \\ \\ 40b + 10p &= 320 \\ 40b + 30p &= 640 \\ \\ \text{Subtract equations:} & \\ 20p &= 320 \\ p &= 16 \\ \\ \text{Substitute back:} & \\ 40b + 10(16) &= 320 \\ 40b + 160 &= 320 \\ 40b &= 160 \\ b &= 4 \end{aligned}\]

loaf of bread: \(\boxed{ \text{\$} 4 }\)

pie: \(\boxed{ \text{\$} 16 }\)
\end{solution}
\newpage

\question[4] Write the augmented matrix for the system of linear equations.\\
    
\(\displaystyle{ \begin{aligned} -5y &= 5 + 21x \\ 0 &= -13y - 20 + 14x \end{aligned} }\)
\begin{solution}
\[\begin{aligned} -21x - 5y &= 5 \\ -14x + 13y &= -20 \end{aligned}\]

\[\boxed{ \left[\begin{array}{cc|c} -21 & -5 & 5 \\ -14 & 13 & -20 \end{array}\right] }\]
\end{solution}
\vspace{\stretch{1}}

\question[6] Perform the row operations on the given matrix.\\
    
\(\displaystyle{ \left[\begin{array}{cc|c} -5 & 12 & -30 \\ 1 & -3 & 6 \end{array}\right] }\)
\begin{solution}
\[\begin{aligned} & \left[\begin{array}{cc|c} -5 & 12 & -30 \\ 1 & -3 & 6 \end{array}\right] \\ \xrightarrow{R_1 \leftrightarrow R_2} & \left[\begin{array}{cc|c} 1 & -3 & 6 \\ -5 & 12 & -30 \end{array}\right] \\ \xrightarrow{R_2 + 5R_1 \to R_2} & \left[\begin{array}{cc|c} 1 & -3 & 6 \\ 0 & -3 & 0 \end{array}\right] \\ \xrightarrow{\frac{R_2}{-3} \to R_2} & \left[\begin{array}{cc|c} 1 & -3 & 6 \\ 0 & 1 & 0 \end{array}\right] \\ \xrightarrow{R_1 + 3R_2 \to R_1} & \left[\begin{array}{cc|c} 1 & 0 & 6 \\ 0 & 1 & 0 \end{array}\right] \end{aligned}\]

\(\boxed{ (6, 0) }\)
\end{solution}
\vspace{\stretch{5}}\answerline
\newpage

\question Find the domain and range of the functions.
\begin{parts}
    \part[5] \(\displaystyle{ g(x) = \displaystyle \frac{5x - 2}{2(x - 5)} }\)
    \vspace{\stretch{1}}\\
    domain:\fillin[][2in] \hspace{\stretch{1}} range:\fillin[][2in]
    \begin{solution}
    \[x - 5 \neq 0 \implies x \neq 5\]

domain: \(\boxed{ x \neq 5 }\)

\[y = \frac{5}{2} \implies y \neq \frac{5}{2}\]

range: \(\boxed{ y \neq \frac{5}{2} }\)
    \end{solution}

    \part[5] \(\displaystyle{ h(x) = (x + 2)(x + 3)(x - 4) }\)
    \vspace{\stretch{1}}\\
    domain:\fillin[][2in] \hspace{\stretch{1}} range:\fillin[][2in]
    \begin{solution}
    \[\text{Domain: } (-\infty, \infty)\]

domain: \(\boxed{ (-\infty, \infty) }\)

\[\text{Range: } (-\infty, \infty)\]

range: \(\boxed{ (-\infty, \infty) }\)
    \end{solution}
\end{parts}
\newpage
\newpage

\question[5] List all of the vertical, horizontal, and oblique asymptotes of \(\displaystyle{ f(x) = \displaystyle{\frac{3x - 1}{4(x + 2)}} }\).\\
\begin{center}
\renewcommand{\arraystretch}{3}
    \begin{tabular}{|c|c|}
        \hline
        \textbf{Asymptotes}  & \textbf{Equation} \\ \hline
        Vertical   & \hspace{50mm} \\ \hline
        Horizontal &  \\ \hline
        Oblique    &  \\ \hline
    \end{tabular}
\end{center}
\begin{solution}
\[\begin{array}{|c|c|} \hline \rule[-0.8em]{0pt}{2.2em} \textbf{Asymptotes} & \textbf{Equation} \\ \hline \rule[-1.2em]{0pt}{3em} \text{Vertical} & x = -2 \\ \hline \rule[-1.2em]{0pt}{3em} \text{Horizontal} & y = \frac{3}{4} \\ \hline \rule[-1.2em]{0pt}{3em} \text{Oblique} & \text{none} \\ \hline \end{array}\]
\end{solution}
\vspace{\stretch{1}}

\question[10] Fill in the table below with the zeros of the polynomial, their multiplicities and the behavior of the graph of the function around each zero. You may or may not use all of the rows in the table.\\
    
    \(\displaystyle{ f(x) = 5 \, x^{4} - 8 \, x^{3} + 3 \, x^{2} }\)\
    
\hspace{\stretch{1}}
\renewcommand{\arraystretch}{3}
\begin{tabular}{|p{20mm}|p{25mm}|p{20mm}|}\hline
\textbf{Zero} & \textbf{Multiplicity} & \textbf{Behavior}\\ \hline
 & & \\ \hline
 & & \\ \hline
 & & \\ \hline
 & & \\ \hline
 & & \\ \hline
\end{tabular}
\begin{solution}
\[\begin{aligned} f(x) &= 5 \, x^{4} - 8 \, x^{3} + 3 \, x^{2} \\ &= x^2(5 \, x^{2} - 8 \, x + 3) \\ &= x^2(5x - 3)(x - 1) \end{aligned}\]

 \[\begin{array}{|c|c|c|} \hline \rule[-0.8em]{0pt}{2.2em} \textbf{Zero} & \textbf{Multiplicity} & \textbf{Behavior} \\ \hline \rule[-1.2em]{0pt}{3em} 0 & 2 & \text{bounce} \\ \hline \rule[-1.2em]{0pt}{3em} \frac{3}{5} & 1 & \text{cross} \\ \hline \rule[-1.2em]{0pt}{3em} 1 & 1 & \text{cross} \\ \hline \rule[-1.2em]{0pt}{3em} & & \\ \hline \rule[-1.2em]{0pt}{3em} & & \\ \hline \end{array}\]
\end{solution}
\newpage

\question[5] Suppose a rational function has the attributes in the table below. Using only the holes, asymptotes, and intercepts listed along with as many of the helpful points as required, sketch the graph of the function.\\
\renewcommand{\arraystretch}{1.5}
    
    \[\begin{array}{|r|c|c|} \hline \textbf{Holes:} & (-7, 0.92) & \\ \hline \textbf{Asymptotes:} & x = -3 & x = 6 \\ & y = -4 & \\ \hline \textbf{Intercepts:} & (-8, 0) & (5, 0) \\ & (0, -8.89) & \\ \hline \textbf{Helpful Points:} & (-9, -0.62) & (2, -6) \\ & (8.5, -8.03) & \\ \hline \end{array}\]
\begin{solution}
\textbf{Instructor Note:} The generated function is: \[f(x) = \frac{-4(x - 5)(x + 8)(x + 7)}{(x + 3)(x - 6)(x + 7)}\] 

 \[\begin{tikzpicture}[scale=0.39] \draw[step=1cm, gray!40, very thin] (-10,-10) grid (10,10); \draw[thick, <->] (-10.5,0) -- (10.5,0); \draw[thick, <->] (0,-10.5) -- (0,10.5); \clip (-10.5,-10.5) rectangle (10.5,10.5);\draw[dashed, blue, thick, <->] (-3, -10.5) -- (-3, 10.5); \draw[dashed, blue, thick, <->] (6, -10.5) -- (6, 10.5); \draw[dashed, blue, thick, <->] (-10.5, -4) -- (10.5, -4); \draw[blue, very thick, samples=80, domain=-10.5:-3.10000000000000, smooth] plot (\x, {-4 * (\x - (5)) * (\x - (-8)) / ((\x - (-3)) * (\x - (6)))}); \draw[blue, very thick, samples=80, domain=-2.90000000000000:5.90000000000000, smooth] plot (\x, {-4 * (\x - (5)) * (\x - (-8)) / ((\x - (-3)) * (\x - (6)))}); \draw[blue, very thick, samples=80, domain=6.10000000000000:10.5, smooth] plot (\x, {-4 * (\x - (5)) * (\x - (-8)) / ((\x - (-3)) * (\x - (6)))}); \fill[blue] (-8.0, 0) circle (4pt); \fill[blue] (5.0, 0) circle (4pt); \fill[blue] (0, -8.89) circle (4pt); \fill[blue] (-9.0, -0.62) circle (4pt); \fill[blue] (2.0, -6.0) circle (4pt); \fill[blue] (8.5, -8.03) circle (4pt); \draw[blue, thick, fill=white] (-7.0, 0.92) circle (4pt); \end{tikzpicture}\]
\end{solution}
\begin{center}
\scalebox{1}{
    \begin{tikzpicture}[>=triangle 45]
      \draw [step=.5cm, style={black!60}] (-5.4,-5.4) grid (5.4,5.4);
      \draw [->, line width=2] (-5.5,0) -- (5.5,0); 
      \draw [->, line width=2] (0,-5.5) -- (0,5.5);
    \end{tikzpicture}
}
\end{center}
\newpage

\question Use the image of a polynomial below to answer the questions that follow.\\
\begin{center}
\begin{tikzpicture}[scale=0.39] \draw[step=1cm, gray!40, very thin] (-10,-10) grid (10,10); \draw[thick, <->] (-10.5,0) -- (10.5,0); \draw[thick, <->] (0,-10.5) -- (0,10.5); \clip (-11,-11) rectangle (11,11); \draw[<->, blue, very thick, samples=150, domain=-4.07:5.48, smooth] plot (\x, {0.088781 * (\x + 3.5) * (\x + 1) * (\x - 3.5) * (\x - 4.5)}); \end{tikzpicture}
\end{center}
\begin{solution}
\begin{itemize}
\item
The degree is even.

\item
The lead coefficient is positive.

\item
The constant is positive.

\item
The minimum degree is 4.

\end{itemize}
 \textbf{(For instructor reference, the function equation is \(0.0888(x + 3.5)(x + 1)(x - 3.5)(x - 4.5)\))}
\end{solution}
\begin{parts}
    \part[1] Is the degree of the polynomial even or odd? \\\\
    \begin{oneparcheckboxes}
    \choice even
    \choice odd
    \end{oneparcheckboxes}\\\vspace{.125in}
    \part[1\half] Is the lead coefficient of the polynomial positive or negative?\\\\
    \begin{oneparcheckboxes}
    \choice positive
    \choice negative
    \end{oneparcheckboxes}\\\vspace{.125in}
    \part[1] Is the constant of the polynomial positive or negative? \\\\
    \begin{oneparcheckboxes}
    \choice positive
    \choice negative
    \end{oneparcheckboxes}\\\vspace{.125in}
    \part[1\half] What is the minimum degree? \\\\\fillin
\end{parts}
\end{questions}
\end{document}
